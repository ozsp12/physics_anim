(function (root, factory) {
  ...
  if (!physics) {
    throw new Error('VerticalLoopPhysics is required');
  }

  function mechanicalEnergyPerMass(state, gravity = physics.G) {
    return 0.5 * state.speed * state.speed
      + gravity * physics.loopHeight(state.radius, state.theta);
  }

  function stepLoopState(state, dt, gravity = physics.G) {
    ...
    const normalAcceleration = physics.normalAccelerationFromState(
      speed,
      state.radius,
      theta,
      gravity
  ...
  function simulateLoop({ height, radius, dt = 1 / 240, gravity = physics.G, maxTime = 20 }) {
    ...
    while (time < maxTime && state.theta < 2 * Math.PI) {
      ...
      const relativeDrift = Math.abs(energy - initialEnergy) / Math.max(Math.abs(initialEnergy), 1e-12);
      maxRelativeEnergyDrift = Math.max(maxRelativeEnergyDrift, relativeDrift);

      if (state.theta <= 0 && state.speed < 0) {
        return { outcome: 'return', state, time, maxRelativeEnergyDrift, minNormalAcceleration };
      }

      if (physics.shouldDetach({
        theta: state.theta,
        speed: state.speed,
        normalAcceleration: state.normalAcceleration
      })) {
